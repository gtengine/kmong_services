...
# ### 포스팅할 카페
# # url frame
# url_frame = LabelFrame(master=root, text="포스팅할 카페 주소")
...
def check_access_token(client_id, client_secret, user_id, user_password):
    ...
    clientConnect = f'{client_id}:{client_secret}'
    ...
    r = requests.get(url, headers={"Authorization": f"Basic {clidst_base64}"})
    ...
    try:
        ...
        for txt in files:
            ...
            for _ in range(0, n_posting):
                ...
                content = urllib.parse.quote(body)
                data = "subject=" + subject + "&content=" + content
                
                request = urllib.request.Request(url, data=data.encode('KSC5601'))
